...
#*****************************************************
#This script pulls OSIsoft PI data for a tag over a specific date range.
...
$digitalState = $myValues.Value.State 
...
for($i=0; $i -lt $valueCount; $i++){
    ...
    $myStreamWriter.WriteLine("$($digitalState[$i]),$($sDate[$i] + " " + $sTime[$i])")
